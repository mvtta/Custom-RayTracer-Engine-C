...
/*   Updated: 2022/06/06 18:40:39 by user             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_image(t_frame *rt, int which)
{
	if (which == BKG)
	{
		rt->bkg_img.img_ptr = mlx_new_image(rt->mlx_ptr,
				rt->window_w, rt->window_h);
		rt->bkg_img.data = (int *)mlx_get_data_addr(rt->bkg_img.img_ptr,
				&rt->bkg_img.bits_per_pixel,
				&rt->bkg_img.line_length,
				&rt->bkg_img.endian);
		return ;
	}
	if (which == OBJ)
