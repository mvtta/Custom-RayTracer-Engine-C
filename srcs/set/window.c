...
/*   Updated: 2022/05/25 11:55:21 by mvaldeta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	data.data[(y * data.line_length + x * \
	(data.bits_per_pixel / 8) / 4)] = color;
}
...
{
	rt->mlx_ptr = mlx_init();
	rt->win_ptr = mlx_new_window(rt->mlx_ptr, rt->window_w, \
	rt->window_h, "make an info buf stored in rt");
	...
	&rt->obj_img.bits_per_pixel, &rt->obj_img.line_length, &rt->obj_img.endian);
...
void	background_to_window(t_frame *rt)
{
	...
	rt->win_ptr, rt->bkg_img.img_ptr, 0, 0);
}

void	obj_to_window(t_frame *rt)
{
	mlx_put_image_to_window(rt->mlx_ptr, \
	rt->win_ptr, rt->obj_img.img_ptr, 0, 0);
}
